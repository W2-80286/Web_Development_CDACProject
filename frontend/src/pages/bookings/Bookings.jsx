...
const BookingDetails = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                setData(response.data.data.map(booking => ({
                    ...booking, // Ensure ID is string for DataGrid
                })));
            } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            setData(data.filter((item) => item.id !== id)); // Use the correct ID field
    ...
    const columns = [
        ...
        { field: 'payment_status', headerName: 'Payment Status', width: 150 }, // Add payment_status field
        {
            field: 'action',
            headerName: 'Action',
            width: 150,
